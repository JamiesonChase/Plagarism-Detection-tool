    ...
    */
    public String translate(String input) {
		...
		for (int i=0; i<phrases.length; i++) {
			if (input.contains(phrases[i])) input=(input.replace(phrases[i], piratetalk[i]));
		}
		for (int i=0; i<3; i++) {
			if (input.contains(positiveWords[i])) input=input+" like me pirate treasure!";
			if (input.contains(negativeWords[i])) input=input+" like bein' food for the fish!";
		}
			return input;
    }
